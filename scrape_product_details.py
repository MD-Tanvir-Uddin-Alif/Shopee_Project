...
def scrape_shopee_product_details(shop_id: int, product_id: int, cookies_json: list = []):
    ...
    try:
        with sync_playwright() as p:
            ...
            try:
                ...
                success = False
                return product_data, success # Early return on failure
            ...
            page_url = f"https://shopee.sg/abc-i.{shop_id}.{product_id}"
            logging.info(f"Opening product page: {page_url}")
            try:
            ...
            for entry in api_requests:
                try:
                    data = entry["response"]
                    if isinstance(data, dict) and 'data' in data and 'item' in data['data']:
                        item = data['data']['item']
                        ...
                        found_product = True
                        logging.info(f"Extracted product data from {page_url}")
                        break  # Since it's a single product, no need to check further
                except Exception as e:
                    logging.error(f"Error processing API entry for {entry.get('url', 'unknown')}: {e}")
                    success = False
            if not found_product:
                logging.warning(f"No product found for: {page_url}")
            # Debug info: if no API requests were captured
            if not api_requests:
                logging.warning("No API requests captured for 'item/get'.")
                try:
        ...
        logging.error(f"Failed to scrape product {product_id} from shop {shop_id}: {e}")
        success = False
