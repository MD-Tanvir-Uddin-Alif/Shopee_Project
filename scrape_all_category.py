...
logging.basicConfig(
    ...
    filename='category_scraping.log',
    filemode='a'
)
...
def extract_categories(categories, extracted=[], parent_catid=None, parent_name=None):
    for cat in categories:
        extracted.append({
            ...
            "parent_catid": parent_catid if parent_catid is not None else cat.get('parent_catid', 0),
    ...
    try:
        with sync_playwright() as p:
            ...
            def capture_response(response):
                ...
                try:
                    if "get_category_tree" in response.url:
            ...
            try:
                ...
                return all_extracted  # Early return on failure
            ...
            page_url = f"https://shopee.sg/all_categories"
            ...
            # Extract categories from captured API responses
            found = False
            for entry in api_requests:
                try:
                    data = entry["response"]
                    if isinstance(data, dict) and 'data' in data and 'category_list' in data['data']:
                        all_extracted = extract_categories(data['data']['category_list'])
                        found = True
                        logging.info(f"Extracted {len(all_extracted)} categories")
                        break
                except Exception as e:
                    logging.error(f"Error processing API entry for {entry.get('url', 'unknown')}: {e}")

            if not found:
                logging.warning("No 'get_category_tree' API found. Try increasing sleep time or check dev tools.")
            
            # Debug info: if no API requests were captured
            if not api_requests:
                logging.warning("No API requests captured for 'get_category_tree'.")
                try:
        ...
        logging.error(f"Unexpected error in Category_Scraper: {e}")
